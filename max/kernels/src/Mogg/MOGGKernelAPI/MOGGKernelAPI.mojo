...
struct Struct_fused_qk_rope_vision:
    ...
    fn _rope[
        ...
        freq_dtype: DType,
        width: Int,
    ](val: SIMD[dtype, width], cos_v: SIMD[freq_dtype, width], sin_v: SIMD[freq_dtype, width]) -> SIMD[dtype, width]:
    ...
    fn execute[
        ...
        comptime compile_target = _current_target() if is_cpu[target]() else get_gpu_target()
        ...
        comptime kernel_simd_width = gcd(target_simd_width, rope_dim) if gcd(target_simd_width, rope_dim) >= 2 else 2
        
        __comptime_assert kernel_simd_width >= 2, "kernel_simd_width must be >= 2 for RoPE deinterleave"
        
        # Define the RoPE worker function for Q output
        @parameter
        @__copy_capture(query, freqs_cos, freqs_sin, q_out, seq_len, head_dim)
        @always_inline
        fn rope_fn_q[width: Int, rank: Int, alignment: Int = 1](idx: IndexList[rank]):
            """Process one element of the Q tensor with RoPE."""
            __comptime_assert rank == 4, "Expected rank 4 for Q tensor"
        ...
        @always_inline
        fn rope_fn_k[width: Int, rank: Int, alignment: Int = 1](idx: IndexList[rank]):
            """Process one element of the K tensor with RoPE."""
            __comptime_assert rank == 4, "Expected rank 4 for K tensor"
        ...
        if is_cpu[target]():
            # CPU path
            elementwise[func=rope_fn_q, simd_width=kernel_simd_width, target=target](q_shape)
            elementwise[func=rope_fn_k, simd_width=kernel_simd_width, target=target](k_shape)
        elif is_gpu[target]():
            # GPU path - pass device context
            var dev_ctx = ctx.get_device_context()
            elementwise[func=rope_fn_q, simd_width=kernel_simd_width, target=target](q_shape, dev_ctx)
            elementwise[func=rope_fn_k, simd_width=kernel_simd_width, target=target](k_shape, dev_ctx)
