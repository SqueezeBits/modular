# ===----------------------------------------------------------------------=== #
# Copyright (c) 2026, Modular Inc. All rights reserved.
#
# Licensed under the Apache License v2.0 with LLVM Exceptions:
# https://llvm.org/LICENSE.txt
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ===----------------------------------------------------------------------=== #

"""Flux2 model-specific step-cache miss adapter."""

from __future__ import annotations

from typing import Any

from max.experimental import functional as F
from max.experimental.tensor import Tensor


class Flux2StepCacheMissAdapter:
    """Compute Flux2 output for cache-miss branch."""

    def __init__(
        self,
        *,
        model: Any,
        new_hidden_states: Tensor,
        new_encoder_hidden_states: Tensor,
        double_stream_mod_img: tuple[
            tuple[Tensor, Tensor, Tensor], tuple[Tensor, Tensor, Tensor]
        ],
        double_stream_mod_txt: tuple[
            tuple[Tensor, Tensor, Tensor], tuple[Tensor, Tensor, Tensor]
        ],
        single_stream_mod: tuple[Tensor, Tensor, Tensor],
        image_rotary_emb: tuple[Tensor, Tensor] | None,
        num_txt_tokens: Any,
        temb: Tensor,
    ) -> None:
        self.model = model
        self.new_hidden_states = new_hidden_states
        self.new_encoder_hidden_states = new_encoder_hidden_states
        self.double_stream_mod_img = double_stream_mod_img
        self.double_stream_mod_txt = double_stream_mod_txt
        self.single_stream_mod = single_stream_mod
        self.image_rotary_emb = image_rotary_emb
        self.num_txt_tokens = num_txt_tokens
        self.temb = temb

    def compute_output(self) -> Tensor:
        """Return model output after running remaining blocks."""
        h = self.new_hidden_states
        enc = self.new_encoder_hidden_states

        for rest_block in self.model.transformer_blocks[1:]:
            enc, h = rest_block(
                hidden_states=h,
                encoder_hidden_states=enc,
                temb_mod_params_img=self.double_stream_mod_img,
                temb_mod_params_txt=self.double_stream_mod_txt,
                image_rotary_emb=self.image_rotary_emb,
            )

        h = F.concat([enc, h], axis=1)
        for i in range(len(self.model.single_transformer_blocks)):
            single_block = self.model.single_transformer_blocks[i]
            h = single_block(  # type: ignore[assignment]
                hidden_states=h,
                encoder_hidden_states=None,
                temb_mod_params=self.single_stream_mod,
                image_rotary_emb=self.image_rotary_emb,
                split_hidden_states=False,
            )

        h = h[:, self.num_txt_tokens :, :]
        h = self.model.norm_out(h, self.temb)
        return self.model.proj_out(h)
