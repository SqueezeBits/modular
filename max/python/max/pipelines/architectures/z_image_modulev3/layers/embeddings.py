...
def _get_1d_rope_interleaved(
    ...
    freqs = F.outer(pos, freq)  # [S, half]
    # Stack as [cos, sin] pairs: [S, half, 2] → [S, dim]
...
class RopeEmbedder(Module[[Tensor], Tensor]):
    """Compute multi-axis RoPE directly in interleaved [cos,sin] pair format.

    Returns ``freqs_cis`` of shape ``[S, sum(axes_dims)]`` ready for
    ``rope_ragged_with_position_ids(interleaved=True)``.
    """
