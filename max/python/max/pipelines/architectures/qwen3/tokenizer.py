...
"""Qwen3-specific tokenizer for Flux2 Klein (chat template from repo)."""
...
class Qwen3Tokenizer(TextTokenizer):
    """Qwen3 tokenizer that loads chat_template from the model repo.

    Used by Flux2 Klein pipeline; same pattern as Mistral3Tokenizer.
    """
    ...
    def _load_and_set_chat_template(
        ...
        """Load chat template from chat_template.json and set it on the tokenizer."""
        if revision is None:
