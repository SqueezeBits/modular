...
class Downsample2D(Module[[Tensor], Tensor]):
    ...
    def __init__(
        ...
        Args:
            ...
            norm_type: Normalization type (currently not supported).
            eps: Epsilon for normalization (currently not used).
            elementwise_affine: Elementwise affine for normalization (currently not used).
        ...
        if norm_type == "ln_norm":
            self.norm = LayerNorm(dim=channels, eps=eps or 1e-5, elementwise_affine=elementwise_affine if elementwise_affine is not None else True)
