...
class DistributedFlux2FeedForward(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.linear_in.weight.sharding_strategy = ShardingStrategy.gate_up(self.num_devices, axis=0)
        ...
        self.linear_out.weight.sharding_strategy = ShardingStrategy.columnwise(self.num_devices)
...
class DistributedFlux2Attention(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.to_out = LayerList([_to_out_linear])  # to_out.0.weight matches checkpoint
        ...
        if added_kv_proj_dim is not None:
            ...
            self.add_q_proj.weight.sharding_strategy = ShardingStrategy.rowwise(n)
            ...
            self.add_k_proj.weight.sharding_strategy = ShardingStrategy.rowwise(n)
            ...
            self.add_v_proj.weight.sharding_strategy = ShardingStrategy.rowwise(n)
    ...
    def _per_device_forward(
        ...
        query = ops.reshape(query, [batch_size, seq_len, self.local_heads, self.head_dim])
        key = ops.reshape(key, [batch_size, seq_len, self.local_heads, self.head_dim])
        value = ops.reshape(value, [batch_size, seq_len, self.local_heads, self.head_dim])
        ...
        if encoder_hidden_states is not None and self.added_kv_proj_dim is not None:
            ...
            enc_q = ops.reshape(enc_q, [batch_size, enc_seq_len, self.local_heads, self.head_dim])
            enc_k = ops.reshape(enc_k, [batch_size, enc_seq_len, self.local_heads, self.head_dim])
            enc_v = ops.reshape(enc_v, [batch_size, enc_seq_len, self.local_heads, self.head_dim])
        ...
        attn_out = ops.reshape(attn_out, [batch_size, seq_len, self.local_inner_dim])
    ...
    def __call__(
        ...
        results = [
            self._per_device_forward(
                ...
                encoder_hidden_states_list[i] if encoder_hidden_states_list is not None else None,
                image_rotary_emb_list[i] if image_rotary_emb_list is not None else None,
            )
        ...
        if encoder_hidden_states_list is not None:
            ...
            return self.allreduce(list(results), signal_buffers)
...
class DistributedFlux2ParallelSelfAttention(Module):
    ...
    def __init__(
        ...
        *,
        ...
        qkv_mlp_sub_dims = [self.inner_dim] * 3 + [self.mlp_hidden_dim] * mlp_mult_factor
        shard_fn = partial(
    ...
    def _per_device_forward(
        ...
        query = ops.reshape(query, [query.shape[0], query.shape[1], self.local_heads, self.head_dim])
        key = ops.reshape(key, [key.shape[0], key.shape[1], self.local_heads, self.head_dim])
        value = ops.reshape(value, [value.shape[0], value.shape[1], self.local_heads, self.head_dim])
