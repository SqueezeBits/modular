...
"""Unified Sequence Parallelism (USP) Flux2 Transformer model loader.

Handles all CP modes: Ulysses-only, Ring-only, and USP (combined).
"""
...
class USPFlux2TransformerModel(ComponentModel):
    """Multi-GPU context-parallel Flux2 transformer (Ulysses / Ring / USP)."""
    ...
    def __init__(
        ...
        # Normalize: 0 means disabled, but device group computation needs >= 1
        self.ulysses_degree = max(self._init_ulysses_degree, 1)
        self.ring_degree = max(self._init_ring_degree, 1)
    ...
    def load_model(self) -> Callable[..., Any]:
        ...
        self.model: Model = self.session.load(
            graph, weights_registry=self.state_dict_values
        )
