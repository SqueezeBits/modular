...
"""Ulysses (context-parallel) Flux2 Transformer model loader."""
...
from .ulysses_flux2 import UlyssesFlux2Transformer2DModel


class UlyssesFlux2TransformerModel(ComponentModel):
    """Multi-GPU context-parallel Flux2 transformer model."""
    ...
    def load_model(self) -> Callable[..., Any]:
        ...
        nn_model = UlyssesFlux2Transformer2DModel(
            self.config, self.device_refs
        )
        # In CP mode, weights are replicated. load_state_dict loads into
        # the primary (device 0) layers. Replicas share the same weights
        # via the Module's weight registry.
        ...
        self.model: Model = self.session.load(
            graph,
            weights_registry=self.state_dict_values,
        )
