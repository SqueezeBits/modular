...
class _WanTensorUniPCScheduler:
    ...
    def _corrector_coefficients(
        ...
        m0_scale = float(
            -alpha_t * h_phi_1
            + alpha_t * b_h * (rhos_c[0] / rk + rhos_c[-1])
        )
    ...
    def _build_step_coefficients(
        ...
        if scheduler.sigmas is None or scheduler.timesteps is None:
            raise ValueError("Scheduler must be initialized with set_timesteps().")
...
class WanPipeline(DiffusionPipeline):
    ...
    def init_remaining_components(self) -> None:
        ...
        self._ready_vae_prewarm_shapes: set[
    ...
    def _compile_cfg_fastpath_helpers(self) -> None:
        ...
        def split_cfg_predictions(
            ...
            positive_prediction = ops.slice_tensor(
                batched_predictions,
                [slice(0, 1), slice(None), slice(None), slice(None), slice(None)],
            )
        ...
        self.cached_duplicate_cfg_latents = max_compile(
            ...
            input_types=[
                TensorType(
                    dtype,
                    shape=[1, self.transformer.config.in_channels, "frames", "height", "width"],
                    device=device,
                )
            ],
        )
        ...
        self.cached_split_cfg_predictions = max_compile(
            ...
            input_types=[
                TensorType(
                    dtype,
                    shape=[2, self.transformer.config.out_channels, "frames", "height", "width"],
                    device=device,
                )
            ],
        )
    ...
    def _compile_denorm_model(self) -> None:
        ...
        input_types = [
            TensorType(
                DType.float32, ["batch", z_dim, "f", "h", "w"], device=device,
            ),
    ...
    def _get_guidance_scale(
        ...
        *,
        ...
        if dtype == DType.bfloat16:
            ...
            scale = Buffer.from_numpy(u16).to(device).view(
                dtype=DType.bfloat16, shape=[1]
            )
        else:
    ...
    def _prepare_prompt_state(
        ...
        if (
            ...
            batched_prompt_embeds_buf = self.cached_concat_cfg_prompt_embeddings(
                prompt_embeds,
    ...
    def _compute_boundary_step_idx(
        ...
        for idx, timestep in enumerate(scheduler_timesteps):
            if self.use_low_noise_transformer(float(timestep), boundary_timestep):
                return idx
        return len(scheduler_timesteps)
    ...
    def _decode_output(
        ...
        self._finish_vae_prewarm(tuple(int(dim) for dim in denorm_latents.shape))
        ...
        target_num_frames = min(decoded_num_frames, int(model_inputs.num_frames))
    ...
    def _run_transformer_forward(
        ...
        *,
        ...
        if (
            ...
            batched_predictions = _as_buffer(transformer_model(
                _as_buffer(
        ...
        noise_pred_buf = _as_buffer(transformer_model(
            latent_model_input,
            dit_timestep,
            prompt_embeds,
            rope_cos,
        ...
        if (
            ...
            assert guidance_scale is not None
            noise_uncond_buf = _as_buffer(transformer_model(
                latent_model_input,
                dit_timestep,
            ...
            return _as_buffer(
                self._guidance_model(
                    noise_pred_buf, noise_uncond_buf, guidance_scale,
                )
            )
    ...
    def get_t5_prompt_embeds_from_hidden(
        ...
        for batch_idx in range(batch_size):
            ...
            prompt_embeds_np[batch_idx, :effective_seq_len, :] = hidden_states_np[
                batch_idx, :effective_seq_len, :
            ]
    ...
    def compute_video_latent_shape(
        *,
        ...
        latent_frames = (
            (adjusted_num_frames - 1) // scale_factor_temporal + 1
        )
    ...
    def denormalize_vae_latents(
        ...
        mean = np.asarray(latents_mean, dtype=np.float32).reshape(1, z_dim, 1, 1, 1)
        std = np.asarray(latents_std, dtype=np.float32).reshape(1, z_dim, 1, 1, 1)
