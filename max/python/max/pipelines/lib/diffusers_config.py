...
class DiffusersComponentConfig(ConfigFileModel):
    ...
    name: str = Field(
        description=(
            "Component name as it appears in model_index.json "
            "(e.g., 'vae', 'transformer')."
        )
    )

    subfolder: Path = Field(
        description="Path to the component's subfolder relative to repo root."
    )

    library: str = Field(
        description=(
            "Library the component class belongs to "
            "(e.g., 'diffusers', 'transformers')."
        )
    )
    ...
    class_name: str = Field(
        description=(
            ...
            "(e.g., 'AutoencoderKL', 'T5EncoderModel')."
        )
    )

    config_path: Path | None = Field(
        default=None,
        description="Path to the component's config.json file, if it exists.",
    )

    weight_paths: list[Path] = Field(
        default_factory=list,
        description="List of safetensor/pytorch weight files for this component.",
    )

    config_dict: dict[str, Any] = Field(
        default_factory=dict,
        description="Parsed contents of the component's config.json.",
    )
...
class DiffusersConfig(ConfigFileModel):
    ...
    pipeline_class: str = Field(
        description="The diffusers pipeline class name (e.g., 'FluxPipeline')."
    )

    diffusers_version: str = Field(
        default="",
        description="The diffusers version that created this repository.",
    )

    components: dict[str, DiffusersComponentConfig] = Field(
        default_factory=dict,
        description="Mapping of component names to their configurations.",
    )

    model_path: Path = Field(
        description="Root path of the model repository.",
    )

    raw_config: dict[str, Any] = Field(
        default_factory=dict,
        description="Raw contents of model_index.json.",
    )

    # HuggingFace repo metadata (for remote repos) - stored as private attrs
    _repo_id: str | None = PrivateAttr(default=None)
    _revision: str | None = PrivateAttr(default=None)
    _cache_dir: str | Path | None = PrivateAttr(default=None)
    _token: str | None = PrivateAttr(default=None)
    _trust_remote_code: bool = PrivateAttr(default=False)
    ...
    def from_huggingface_repo(
        ...
        Raises:
            OSError: If the repository or model_index.json cannot be found.
        """
        try:
        ...
        # The model_index_file path is in the HF cache structure
        # Parent directory contains the snapshot
        model_path = Path(model_index_file).parent

        config = cls._from_config_dict(raw_config, model_path)

        # Store HF repo metadata for later use
    ...
    def _from_config_dict(
        ...
        diffusers_version = raw_config.get("_diffusers_version", "")
    ...
    @property
    def all_weight_paths(self) -> list[Path]:
        """Get all weight file paths from all components.

        Returns a flat list of all weight files across all components.
        Useful for populating MAXModelConfig.weight_path.

        Returns:
            List of all weight file paths from all components.
        """
        paths: list[Path] = []
        for component in self.components.values():
            paths.extend(component.weight_paths)
        return paths
    ...
    def get_component_config(
        ...
        Returns:
            The component's config dict, or None if not found.
        """
        component = self.components.get(component_name)
        if component is None:
            return None
