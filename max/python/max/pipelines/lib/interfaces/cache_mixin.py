...
from max.graph import TensorType, TensorValue, DeviceRef
...
def can_use_fbcache(
    ...
    return F.transfer_to(F.squeeze(pred, 0), DeviceRef.CPU())
...
def teacache_conditional_execution(
    ...
    should_skip = F.transfer_to(F.squeeze(~force_compute & (next_accumulated < thresh), 0), DeviceRef.CPU())
