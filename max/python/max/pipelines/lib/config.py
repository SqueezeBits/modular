...
class ImageGenerationConfig(PipelineConfig):
    model_path: str = Field(default="")
    """The repo id of the image generation model."""

    @staticmethod
    def help() -> dict[str, str]:
        return {
            "model": "The path to the image generation model.",
        }

    @classmethod
    def from_flags(
        cls, flags: dict[str, str], **config_flags: Any
    ) -> ImageGenerationConfig:
        merged_configs = {**flags, **config_flags}
        return cls(**merged_configs)

    def log_basic_config(self) -> None:
        logger.info(f"model_info: {self.model}")
        logger.info("")