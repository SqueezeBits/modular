...
class MAXModelConfig(MAXModelConfigBase):
    ...
    def set_default_cache_dtype_if_needed(self) -> None:
        """Set the default KV cache dtype based on quantization encoding if not already set.

        This method should only be called after the quantization encoding has
        been resolved. It sets the cache dtype based on the quantization encoding,
        unless kv_cache_format is already set (which takes precedence).

        For diffusion models, this method does nothing as they don't use KV cache.
        """
        from .hf_utils import is_diffusion_pipeline

        # Skip for diffusion models as they don't use KV cache
        # Only check if model_path is set (not empty)
        if self.model_path and is_diffusion_pipeline(self.huggingface_model_repo):
            return

        # If kv_cache_format is already set, use it
        if self.kv_cache.kv_cache_format is not None:
            ...
            if self.kv_cache.kv_cache_format in cache_dtype_map:
                object.__setattr__(
                    self.kv_cache, "_cache_dtype", cache_dtype_map[self.kv_cache.kv_cache_format]
                )
            return

        # Set default cache dtype based on quantization encoding
        if not self.quantization_encoding:
            return

        # Map quantization encoding to cache dtype
        encoding_to_cache_dtype = {
            SupportedEncoding.bfloat16: DType.bfloat16,
            SupportedEncoding.float16: DType.float16,
            SupportedEncoding.float8_e4m3fn: DType.float8_e4m3fn,
        }

        cache_dtype = encoding_to_cache_dtype.get(
            self.quantization_encoding, DType.float32
        )
        object.__setattr__(self.kv_cache, "_cache_dtype", cache_dtype)
