...
class OutputVideoContent(BaseModel):
    ...
    format: str | None = Field(
        None,
        description="The format of the video (e.g., 'mp4', 'webm', 'gif').",
    )
    frames_per_second: int | None = Field(
        None, description="Frame-rate of the generated video."
    )
    num_frames: int | None = Field(
        None, description="Number of frames in the generated video."
    )
    width: int | None = Field(
        None, description="Frame width in pixels."
    )
    height: int | None = Field(
        None, description="Frame height in pixels."
    )

    @classmethod
    def from_encoded_bytes(
        cls,
        data: bytes,
        *,
        format: str = "mp4",
        ...
        """Create OutputVideoContent from pre-encoded video bytes."""
        return cls(
            type="output_video",
            video_data=base64.b64encode(data).decode("utf-8"),
            format=format.lower(),
    ...
    def from_numpy_frames(
        ...
        *,
        ...
        """Create OutputVideoContent from frame tensors of shape ``[F, H, W, C]``."""
        if frames.ndim != 4:
            raise ValueError(
                f"Expected frames with shape [F, H, W, C], got {frames.shape}."
            )
        ...
        if frames.shape[-1] not in (1, 3, 4):
            raise ValueError(
                "Expected channel dimension C to be one of (1, 3, 4)."
            )
        ...
        if frames.dtype == np.float32:
            ...
            raise ValueError(
                f"Unsupported dtype {frames.dtype}. Expected float32 or uint8."
            )
        ...
        if fmt == "gif":
            ...
            for frame in frames_uint8:
                if frame.shape[-1] == 1:
                    pil_frames.append(
                        Image.fromarray(frame.squeeze(-1), mode="L")
                    )
            ...
            first.save(
                ...
                duration=max(1, int(1000 / frames_per_second)),
                loop=0,
            )
            ...
            # Prefer PyAV when available: produces proper MP4/WEBM bytes in-memory.
            try:
                import av
                ...
                with av.open(buffer, mode="w", format=fmt) as container:
                    stream = container.add_stream(codec, rate=frames_per_second)
                    stream.width = int(frames_uint8.shape[2])
                    stream.height = int(frames_uint8.shape[1])
                    if fmt == "mp4":
                        stream.pix_fmt = "yuv420p"
                        # Avoid heavy macroblock artifacts from low default bitrate.
                        stream.bit_rate = int(
                            max(
                                8_000_000,
                                stream.width
                ...
                errors.append(f"pyav: {exc!s}")
                encoded = b""

            # Fallback to imageio temp-file encoding if PyAV is unavailable.
            if not encoded:
                try:
                    ...
                    errors.append(f"imageio: {exc!s}")

            if not encoded:
                raise ValueError(
                    "Encoding mp4/webm failed. "
                    + " | ".join(errors)
                )
        else:
            raise ValueError(
                f"Unsupported video format '{format}'. Supported: mp4, webm, gif."
            )

        return cls.from_encoded_bytes(
            encoded,
            format=fmt,
            frames_per_second=frames_per_second,
            num_frames=int(frames_uint8.shape[0]),
