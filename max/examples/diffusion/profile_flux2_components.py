...
from max.pipelines.architectures.flux2.pipeline_flux2 import Flux2Pipeline, Flux2ModelInputs, Flux2PipelineOutput
...
class ProfiledFlux2Pipeline(Flux2Pipeline):
    ...
    def execute(
        ...
        dt_tensors_drv = []
        if hasattr(self, "_scheduler_step_model"):
        ...
        # Build scheduler step graph if not already built (optimized path only)
        if hasattr(self, "_scheduler_step_model") and self._scheduler_step_model is None:
            device = self.transformer.devices[0]
        ...
        for i in tqdm(range(num_timesteps), desc="Denoising"):
            ...
            if hasattr(self, "_scheduler_step_model"):
                ...
                # Legacy uncompiled step: (latents, noise_pred, sigmas, step_index)
