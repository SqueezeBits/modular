...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional cap on the number of conditioning video frames to use. "
            "If unset, the full pose/face video length is used."
        ),
    )
    parser.add_argument(
...
def maybe_limit_video_frames(
    frames: list[Any], max_frames: int | None, label: str
) -> list[Any]:
    """Optionally cap a decoded video frame list."""
    if max_frames is None or len(frames) <= max_frames:
        return frames

    print(f"Limiting {label} from {len(frames)} frames to {max_frames} frames")
...
class IntermediateDumper:
    ...
    def patch_pipeline(self, pipe: WanAnimatePipeline) -> None:
        ...
        if hasattr(transformer, "motion_encoder"):
            ...
            h = transformer.motion_encoder.register_forward_hook(motion_hook)
...
def run(args: argparse.Namespace) -> Path:
    ...
    if args.preprocess:
        ...
        pose_video = maybe_limit_video_frames(
            pose_video, args.num_frames, "pose video"
        )
        face_video = maybe_limit_video_frames(
            face_video, args.num_frames, "face video"
        )
        if args.save_preprocessed:
            save_preprocessed(pose_video, face_video, args.save_preprocessed)
        print(
    ...
    if args.dump_intermediates and args.torch_compile:
        print(
            "Disabling --torch-compile because --dump-intermediates "
            "registers transformer hooks that trigger TorchDynamo "
            "recompiles in Wan Animate."
        )
    ...
    if args.mode == "replace":
        background_video = load_video(args.background_video)
        mask_video = load_video(args.mask_video)
        background_video = maybe_limit_video_frames(
            background_video, args.num_frames, "background video"
        )
        mask_video = maybe_limit_video_frames(
            mask_video, args.num_frames, "mask video"
        )
    ...
    if args.profile_timings:
        ...
        if args.torch_compile:
            ...
            if (
                ...
                # CLIP image encoding in the current Transformers stack
                # triggers Dynamo unsupported operations under fullgraph=True.
                # Allow graph breaks here so we can still compile the module
                # without patching site-packages.
